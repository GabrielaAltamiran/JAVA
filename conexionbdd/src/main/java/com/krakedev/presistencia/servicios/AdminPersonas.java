...
public class AdminPersonas {
		...
		try {
			...
			ps = con.prepareStatement(
					"insert into personas(cedula,nombre,apellido,numero_hijos,estatura,cantidad_ahorrada,fecha_nacimiento,hora_nacimiento,codigo_estado_civil)"
							+ "values (?,?,?,?,?,?,?,?,?)");
			ps.setString(1, persona.getCedula()); // El insert en pgAdmin4 debe estar eliminado o no debe tener la misma
													// llave pk
			ps.setString(2, persona.getNombre());
			ps.setString(3, persona.getApellido());
			ps.setInt(4, persona.getNumero_hijos());
			ps.setDouble(5, persona.getEstatura());
			ps.setBigDecimal(6, persona.getCantidadAhorrada()); // Ayuda para cuando es una valor Money usamos "new
																// BigDecimal(12098.98)"
			...
			try {
				...
				throw new Exception("Error con la base de dato"); // Aqui va ya que no es solo "Exception" si no es
																	// "SQLException" y creamo una nueva.
			}
		}
	}
	// ----------------------ACTUALIZAR------------------------------------
	...
	public static void actualizar(Persona persona) throws Exception {
		...
		try {
			con = Conexionbdd.conectar();
			ps = con.prepareStatement(
					"update personas set nombre=?,apellido=?,numero_hijos=?,estatura=?,cantidad_ahorrada=?,fecha_nacimiento=?,hora_nacimiento=?,codigo_estado_civil=? where cedula=?");
			ps.setString(1, persona.getNombre());
			ps.setString(2, persona.getApellido());
			ps.setInt(3, persona.getNumero_hijos());
			ps.setDouble(4, persona.getEstatura());
			ps.setBigDecimal(5, persona.getCantidadAhorrada()); // Ayuda para cuando es una valor Money usamos "new
																// BigDecimal(12098.98)"
	...
	//-------------------BUSQUEDAD QUE RETORNE VARIOS REGISTROS-------------------------
	...
	//-------------------BUSQUEDAD QUE RETORNE REGISTROS CON CLAVE PRIMARIA-------------------------
	...
	public static Persona buscarPorCedula(String cedula) throws Exception {
		...
		try {
			...
			LOGGER.error("Error al buscar con cedula", e);
			throw new Exception("Error al buscar con cedula");
		} finally {
			try {
				con.close();
			} catch (SQLException e) {
